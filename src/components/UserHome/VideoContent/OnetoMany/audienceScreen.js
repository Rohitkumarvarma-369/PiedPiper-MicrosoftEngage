...
const AudienceScreen = () => {
  const {publicKey} = useParams();//taking the parametrs from the url
  const pubVideo = useRef();
  const subVideo = useRef();

  let isPub, client, signal;

  const config = {//setting the stun and turn servers
    iceServers: [
      {
  ...
  const URL = new URLSearchParams(window.location.search).get("publish");//confirming as broadcaster if publish==true
  console.log("url", URL);
  if (URL) {
    isPub = true;
  } else {
  ...
  useEffect(() => {
    signal = new IonSFUJSONRPCSignal("wss://www.pipertalkserver.codes/ws");//connect to hosted public ip
  ...
  const start = (event) => {//start stremaing accoridng to video or screen selection
    if (event) {
